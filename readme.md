...
1. В консоли измените каталоги на верхний уровень распакованного проекта. _Например, если вы использовали имя проекта по умолчанию,
 `akka-quickstart-scala` и извлекли проект в корневой каталог из корневого каталога, введите: 
 
 ```sbtshell
    cd C:\Users\Desktop\projects\Akka
 ```
...
2. Введите 
 ```sbtshell
   sbt.bat
 ```
 в Windows, чтобы запустить [sbt](https://www.scala-sbt.org/1.x/docs/index.html) или `./sbt` на OSX/Linux. 
 _`sbt` загружает зависимости проекта. `sbt:Akka>` показывает что `sbt` запущен в интерактивном режиме._
