...
2. Перезапустите подчиненную, очистив ее накопленное внутреннее состояние
3. Остановить подчиненную навсегда
4. Эскалируйте сбой, тем самым потерепеть неудачу самому
...
все его подчиненные, перезапуск актора влечет за собой перезапуск всех его подчиненных (но см. ниже для более подробной 
информации), аналогичным завершением актора также прекращается все его подчиненные. Следует отметить, что поведение по 
умолчанию для кэша preRestart класса Actor заключается в том, чтобы завершить все его дочерние элементы перед 
перезапуском, но этот крючок можно переопределить; рекурсивный перезапуск применяется ко всем детям, оставшимся после 
выполнения этого крючка.
...
рассуждать, поэтому рекомендуемым способом в этом случае является повышение уровня надзора.

Akka реализует определенную форму под названием «родительский надзор». Акторы могут создаваться только другими акторами - 
...
Актор, с которым, вероятно, больше всего взаимодействует, является родителем всех созданных пользователем акторов, 
опекуном с именем `/user`. Акторы, созданные с помощью `system.actorOf()`, являются дочерними элементами этого актора. 
Это означает, что, когда этот опекун прекратится, все нормальные акторы в системе также будут отключены. Это также 
означает, что стратегия надзирателя этого опекуна определяет, как контролируются нормальные субъекты верхнего уровня. 
Начиная с Akka 2.1, это можно настроить, используя настройку `akka.actor.guardian-supervisor-strategy`, которая берет 
полностью квалифицированное имя класса `SupervisorStrategyConfigurator`. Когда опекун эскалирует сбой, ответ корневого 
...
Этот специальный опекун был введен для того, чтобы добиться упорядоченной последовательности выключения, где ведение 
...
целью которого является прекращение ребенка на любом типе исключения. Все другие спецэффекты будут эскалированы ... 
но кому? Поскольку у каждого реального актора есть супервизор, супервайзер корневого опекуна не может быть настоящим 
актором. И поскольку это означает, что это «вне пузыря», это называется «пузырь-ходок». Это синтетический `ActorRef`, 
...
системы как `true`, как только опекун корня полностью завершается (все рекурсивно останавливаются дети).
...
* Системная (то есть программирование) ошибка для конкретного полученного сообщения
* (Переходный (`Transient`)) отказ какого-либо внешнего ресурса, используемого при обработке сообщения
...
внутреннее состояние необходимо очистить. Если надзорный орган решает, что на его других детей или на себя это не влияет 
коррупция. Из-за сознательного применения шаблона ядра ошибки - поэтому лучше всего перезапустить дочерний элемент. 
...
2. вызовите привязку preRestart старого экземпляра (по умолчанию отправляет запросы завершения для всех детей и 
вызывается postStop);
3. подождите, пока все дети, которым было предложено прекратить (используя context.stop ()) во время preRestart, 
...
5. invoke postRestart в новом экземпляре (который по умолчанию также вызывает preStart);
...
`Akka.pattern.BackoffOptions` можно использовать для настройки поведения отступающего актора-наблюдателя, ниже приведены
 некоторые примеры:
...
использует стратегию остановки по умолчанию, любое исключение приведет к остановке ребенка.
 ...
 Существует два класса стратегий надзора, которые поставляются с Akka: `OneForOneStrategy` и `AllForOneStrategy`. 
