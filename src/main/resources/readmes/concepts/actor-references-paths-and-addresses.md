...
#### Поиск акторов по бетонному пути
...
просмотреть выбор для изменения живота. Чтобы это сделать, устраните неопределенность, отправив запрос и собрав все ответы, 
...
Равенство `ActorRef` соответствует намерению, что `ActorRe` соответствует воплощению целевого актора. Сравнение двух 
акторов сравнивается, когда они имеют один и тот же путь и указывают на одно и то же воплощение актора. Ссылка, указывающая
 на завершенного актора, не сравнивается с ссылкой, указывающей на другого (воссозданного) актора с тем же путем. 
...
### Повторное использование контуров актора
Когда актор прекращается, его ссылка укажет на почтовый ящик мертвой буквы, `DeathWatch` опубликует его окончательный 
переход и, как правило, он не оживет вернуться к жизни (так как жизненный цикл актора не позволяет этого). В то время 
...
слушателей или участников, автоматически развернутых по конфигурации в начале системы акторовl
* `/deadLetters` - это агент мертвой буквы, в котором все сообщения, отправленные остановленным или несуществующим 
субъектам, перенаправляются (с максимальной эффективностью: сообщения могут быть потеряны даже в пределах локальной JVM);
* `/temp` является хранителем для всех недолговечных созданных системой субъектов, например. те, которые используются 
