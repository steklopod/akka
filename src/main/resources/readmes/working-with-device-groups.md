...
функций не углубляясь в детали, которые нам понадобятся для построения позже.

### Добавление поддержки регистрации для акторов устройства
...
не принимаются сообщения, это утверждение проходит. Обычно рекомендуется устанавливать эти таймауты небольшими (но не слишком),
...
* Обработка запроса на регистрацию путем пересылки его существующему игроку устройства или путем создания нового участника 
и пересылки сообщения;
...
Мы также хотим сохранить идентификатор оригинального отправителя запроса, чтобы наш актор устройства мог напрямую ответить.
 Это возможно, используя **`forward`** вместо **`!`** оператор. Единственное различие между ними заключается в том, что 
 **`forward`** хранит  оригинального отправителя **`!`** устанавливает отправителя как текущего участника. Так же, как с 
 нашим игроком на устройстве,  мы гарантируем, что мы не будем реагировать на неправильные идентификаторы групп. 
 Добавьте в исходный файл следующее:
...
может либо явно обработать это сообщение, либо потерпеть неудачу с **`DeathPactException`**. Это полезно, 
...
2. Удаляет актор устройства с `Map[String, ActorRef]` - которая отображает устройства для участников устройства - когда 
...
чтобы удалить его с отображения существующих устройств на сопоставления акторов устройства. Чтобы иметь возможность сделать это 
...
* Чтобы **остановить актор** устройства из нашего тестового примера. С внешней стороны любой актор может быть остановлен, 
отправив специальное встроенное сообщение **`PoisonPill`**, которое инструктирует актор остановиться;
...
исходном файле **`DeviceManager`**. Этот актор очень похож на актора группы устройств, но **создает актора группы устройств**, 
а не актора устройства:
