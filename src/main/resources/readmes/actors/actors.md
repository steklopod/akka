...
После остановки актера вызывается его крюк `postStop`, который может быть использован, например. за отмену регистрации
 этого участника из других служб. Этот крючок гарантированно будет запущен после того, как очередь сообщений будет 
 отключена для этого актера, то есть сообщения, отправленные остановленному игроку, будут перенаправлены на 
