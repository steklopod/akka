...
После остановки актора вызывается его крюк `postStop`, который может быть использован, например. за отмену регистрации
 этого участника из других служб. Этот крючок гарантированно будет запущен после того, как очередь сообщений будет 
...
```scala
// будем смотреть всем детям на службу B с именами, начинающимися с рабочего
...
>Есть последствия для использования использования `ask`, потому что что-то нужно отслеживать, когда оно истекает, 
должно быть что-то, что превращает `Promise` в `ActorRef`, и оно также должно быть доступно через удаленный доступ. 
Поэтому всегда предпочитайте говорить о производительности, и только спросите, должны ли вы.
...
комбинацией. Обратите внимание, что все вышеперечисленное полностью не блокирует и асинхронно: `ask` производит `Future`,
